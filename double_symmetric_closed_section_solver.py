...
class MultiClosedSectionSolver(OpenSectionSolver):
    ...
    def calculate_rate_of_twist_eqn(self):

        for i, element in enumerate(self.shape.sections):
            section_rate_of_twist_eqn = 0
            q = symbols(f'q{i}', real=True)
            for element in self.shape.elements.values():
                if element.pos not in self.shape.sections[i]:
                    continue
                section_rate_of_twist_eqn += element.Q / element.t
                section_rate_of_twist_eqn += q / element.t
            self.shape.rate_of_twist_eqn.append(section_rate_of_twist_eqn)
